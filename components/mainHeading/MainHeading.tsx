...
export default function MainHeading({
  ...
  return (
    <Tag className={`flex items-start ${className}`}>
      <Level
        className={`font-display mr-2 font-bold leading-[1.2] text-white 
        ${sizeClasses[size]}`}
        {...rest}
      >
        {children}
      ...
      {number !== "" && (
        <span className="font-display block text-base font-semibold leading-[1.2] text-white md:text-lg lg:text-xl">
          {number}
        </span>
      )}
    </Tag>
